...
URL = "http://192.168.1.103:1234/v1/chat/completions"
system_prompt = """
...
— Текст должен быть кратким. Не более 400 символов!!!
— Не просто сожми текст, а пойми суть и передай ее своими словами.
— **Жирным выделяй только заголовок и максимум 1-2 ключевых слова в каждом абзаце.** Не злоупотребляй форматированием.
— Каждый пост должен начинаться с понятного, простого и интересного заголовка (до 10 слов), выделенного с помощью **.
...
def ask_model(text: str) -> str:
    ...
    logger.debug("Начинаю обработку через AI")
    ...
    logger.info("Обработка через AI завершена")
    return content_text
